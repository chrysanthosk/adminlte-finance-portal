...
NODE_MAJOR=20
APP_DIR=$(pwd)
DEPLOY_DIR="/var/www/finance-portal"
...
print_info "Granting table permissions to application user..."
...
print_info "Copying built files to web root: $DEPLOY_DIR"
mkdir -p "$DEPLOY_DIR"
...
print_info "Starting or restarting API server with PM2..."
cd "$APP_DIR/api"
pm2 restart index.js --name "finance-api"
pm2 save
pm2 startup
...
echo -e "================================================================"
echo
echo -e "Your Finance Portal is now running."
if [ "$USE_SSL" == "true" ]; then
    echo -e "You can access it at: \e[1;36mhttps://${DOMAIN_NAME}\e[0m"
else
    echo -e "You can access it at: \e[1;36mhttp://${DOMAIN_NAME}\e[0m"
fi
echo
echo -e "To ensure your API server restarts automatically on server reboot, please"
echo -e "run the following command (generated by PM2), then press Enter:"
echo
echo -e "\e[1;33m"
...
echo -e "  - To view API logs: \e[1;36mpm2 logs finance-api\e[0m"
echo -e "  - To stop the API: \e[1;36mpm2 stop finance-api\e[0m"
echo -e "  - To restart the API: \e[1;36mpm2 restart finance-api\e[0m"
